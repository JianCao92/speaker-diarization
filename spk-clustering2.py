...
def spk_cluster_in(features, recline, speakers, outf, dist=bic, segf=None):
    ...
    # TODO: This function has different semantics than spk_cluster_hi, refactor
    ...
    if mind <= threshold:
        ...
        # best_candidate + 1 because we want speakers_X >= 1 in the output
...
def spk_cluster_hi(features, recipe, speakers, outf, dist=bic, segf=None):
    ...
    global max_dist, min_dist
    sp = len(speakers)
    distances = np.full((sp, sp), np.inf)
    ...
    for s1 in xrange(sp):
        for s2 in xrange(s1 + 1, sp):
            ...
            distances[s1, s2] = dist(arr1, arr2)
    while True:
        # Get min d
        mind = distances.min()
        if mind <= threshold or (args.max_spk > 0
                                 and len(speakers) > args.max_spk):
            # Under threshold, fuse speakers!!
            index = distances.argmin()
            best_candidates = [index / len(speakers), index % len(speakers)]
            # Reorder, min first (we don't populate the other part of the
            # matrix)
            if best_candidates[0] > best_candidates[1]:
                best_candidates[0], best_candidates[1] = \
                    best_candidates[1], best_candidates[0]
            ...
            speakers.pop(best_candidates[1])  # List reorder, inefficient
            # Recalculating new speaker distances vs rest
            s1 = best_candidates[0]
            s1b = best_candidates[1]
            # s1b is "out"
            distances = np.delete(distances, s1b, axis=0)
            distances = np.delete(distances, s1b, axis=1)
            # Get new speaker features (s1 is now s1 + s1b)
            arr1 = get_spk_features(speakers[s1], features)
            # And its distances with the rest
            ...
            # Convergence achieved, record statistics
            # TODO: distances.max() will return sys.maxint with this,
            # initialization, fix
    ...
    speaker_turns = [(speaker, turn) for speaker, turns in enumerate(speakers) for turn in turns]
    speaker_turns.sort(key=itemgetter(1))
    for speaker, turn in speaker_turns:
        write_recipe_line(recipe[turn[2]], turn[0], turn[1], 0, speaker + 1, outf,
                          segf)
